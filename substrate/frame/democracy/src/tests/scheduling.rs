...
fn simple_passing_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			tally(r),
			Tally { ayes: 100, nays: 0, turnout: 100, aye_voters: 10000, nay_voters: 0 }
		);
...
fn simple_failing_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			tally(r),
			Tally { ayes: 0, nays: 100, turnout: 100, aye_voters: 00000, nay_voters: 10000 }
		);
...
fn delayed_enactment_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			tally(r),
			Tally { ayes: 2100, nays: 0, turnout: 2100, aye_voters: 60000, nay_voters: 0 }
		);
